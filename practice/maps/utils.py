...
def map_and_filter(s, map_fn, filter_fn):
    ...
    [1, 9, 25]
    """
    """ 根据要求 只是把if判断部分写成了lambda函数，当然结果也是lambda函数"""
    # BEGIN Question 0
...
def key_of_min_value(d):
    ...
    'c'
    """
    """功能是返回最小value的key， 因为min函数是比较是key还是value
    返回key的最小值，比较的是value
    """
    # BEGIN Question 0
    # return min('REPLACE THIS WITH YOUR SOLUTION')
    # return min((zip(d.values(), d.keys())))[1]
...
def mean(s):
    ...
    # BEGIN Question 1
    "*** YOUR CODE HERE ***"
    assert len(s) > 0, 's must not be null'
    return sum(s) / len(s)
    # END Question 1
